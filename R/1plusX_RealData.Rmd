...
prepData <- function(files) {
  ...
  columns = c("Clicks", "Impressions", "RecommendationSizePercent","TotalImpressions","Method", "Alpha","Timestamp")
...
files = c("Regression", "LinUCB_Disjoint", "TS_Lin")
data = prepData(files)
...
ggplot(data, aes(x=Timestamp, y=Percent, colour=Factor)) + 
  geom_line() +
#  stat_smooth(aes(x=Timestamp, y=Percent), method = "lm", formula = y ~ poly(x, 11), se = FALSE) +
    xlab("Impressions") + ylab("% of Clicks") +
...
data = prepData(files)
ggplot(data, aes(x=Timestamp, y=Percent, colour=UserPercentage)) + 
  geom_line() +
...
data = data[data$RecommendationSizePercent == 0.02 & data$Alpha == 0.01,]
ggplot(data, aes(x=Timestamp, y=Percent, colour=Factor)) + 
  geom_line() +
#    stat_smooth(aes(x=Timestamp, y=Percent), method = "lm",formula = y ~ poly(x, 10), se = FALSE) +
    xlab("Impressions") + ylab("% of Clicks") +
    expand_limits(y=c(0,1.2)) + 
 #   scale_x_continuous(labels=formatterM()) + 
...
path = "~/Documents/ETH/Thesis/1plusX/Data/Thesis/1plusx/Results/CTR/"
