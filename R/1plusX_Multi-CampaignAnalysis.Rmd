...
data = subset(all, Method == "LinUCB_Disjoint")
data$Ratio = data$Clicks / data$Impressions
p2 = getCampaignComparisonPlot(data, "CTR", "LinUCB Single Campaign", limit) 

data = subset(all, Method == "LinUCB_Disjoint_Multi_Mix" & Alpha == 0.001)
data$Ratio = data$Clicks / data$Impressions
p3 = getCampaignComparisonPlot(data, "CTR", "LinUCB Multi-Campaign", limit) 
...
p5 = grid.arrange(arrangeGrob(p1 + theme(legend.position="none"),
                               ...
                               nrow=1, ncol=plot_count, widths=unit(rep(9, plot_count),"cm"), heights=unit(12, "cm")),
                   mylegend, nrow=2, widths=unit(plot_count * 9, "cm"), heights=unit(c(12, 1), "cm"),
                  top = textGrob("20% Recommendation CTR Performance", gp=gpar(fontsize=15,fontface=2)))

#p5
ggsave(paste(imageLocation, "S_VS_Multi_CTRPerCampaign.png", sep=""), plot=p5, width = 36, height = 16, units = "cm")
...
data = subset(all, Method == "LinUCB_Disjoint_Multi_Target" & Alpha == 0.1 & RecommendationPart == "NO_SPLIT_1_True")
data$Ratio = data$Impressions / sum(data$Impressions)
...
path = "~/Documents/ETH/Thesis/1plusX/Data/Thesis/1plusx/Results/5/"
files =  c("LinUCB_Disjoint_7days", "Random_7days", "LinUCB_Disjoint_Multi_Mix_7days")
data = prepData(files, path)
...
path = "~/Documents/ETH/Thesis/1plusX/Data/Thesis/1plusx/Results/5/"
files =  c("LinUCB_Disjoint_Multi_7days", "Random_Multi_7days", "LinUCB_Disjoint_Multi_Target_7days")
data = prepData(files, path)
...
#data$Factor = factor(paste(data$Method, data$Alpha, data$TargetSplit, data$EarlyUpdate))
data$Factor = factor(data$Method)
data = subset(data, data$TargetSplit == "NO_SPLIT" | is.na(data$TargetSplit))
#data$Percent = data$BatchCTR * 100
g1 = getCTRPlot(data, c(0,0.6)) 
