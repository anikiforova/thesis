...
print(transformed_users.shape)
index = 0
for line in input:
	line_split = line.split(",")
	norm = transformed_users[index]/ np.linalg.norm(transformed_users[index])
	user_str = np.array2string(norm, precision=3, separator=' ', suppress_small=True)[1:-1].replace('\n', '')
