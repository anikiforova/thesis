...
for testMeta in testsMeta:
	...
	recommendations_per_campaign = dict(zip(campaign_ids, np.zeros(len(campaign_ids))))
	...
	for date in days:
		...
		for cur_file_impressions, line in enumerate(input):
			...
			if (timestamp - hour_begin_timestamp).seconds >= testMeta.get_time_between_updates_in_seconds():			
				...
				algo.update(batch_campaign_ids, batch_users, batch_clicks, discard_target_impressions)
			...
			if assigned_campaign == displayed_campaign:	
				...
				recommendations_per_campaign[assigned_campaign] += 1
			else:
		...
		input.close()
		algo.update(batch_campaign_ids, batch_users, batch_clicks, False)
		algo.log_budgets(log_output, total_impressions, timestamp_raw)
		batch_campaign_ids 	= list()
		batch_users			= list()
		batch_clicks 		= list()
